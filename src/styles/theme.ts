import type { Theme } from '@types'

export const lighTheme: Theme = {
  appBackground: '#FFF',
  appColor: '#000',
  appDefaulStroke: '#E0E0E0',
  appLogo: 'dnc-logo-black.svg',
  appSkeletonFrom: '#EEE',
  appSkeletonTo: '#CCC',
  buttons: {
    alert: '#E80000',
    alertColor: '#FFF',
    alertHover: '#D80000',
    disable: '#CCC',
    desableColor: '#666',
    primary: '#0C70F2',
    primaryColor: '#FFF',
    primaryHover: '#0061DE',
  },
  card: {
    alert: '#E80000',
    background: '#FFF',
    border: '#E0E0E0',
    success: '#008000',
    warning: '#F7A300',
  },
  textInput: {
    active: '#FFF',
    activeColor: '#000',
    borderColor: '#E0E0E0',
    desabled: '#EEE',
    deisabledBorderColor: '#E0E0E0',
    deisabledColor: '#666',
    pleceHolderColor: '#666',
  },
  typographies: {
    error: '#FF0202',
    subtitle: '#666',
    success: '#008000',
  },
}

export const darkTheme: Theme = {
  appBackground: '#060B26',
  appColor: '#FFF',
  appDefaulStroke: '#21497D',
  appLogo: 'dnc-logo-white.svg',
  appSkeletonFrom: '#060B26',
  appSkeletonTo: '#21497D',
  buttons: {
    alert: '#E80000',
    alertColor: '#FFF',
    alertHover: '#D80000',
    disable: '#313649',
    desableColor: '#6D7B8E',
    primary: '#0C70F2',
    primaryColor: '#FFF',
    primaryHover: '#0061DE',
  },
  card: {
    alert: '#E80000',
    background: '#0F1535',
    border: '#21487D',
    success: '#008000',
    warning: '#F7A300',
  },
  textInput: {
    active: '#0F1535',
    activeColor: '#FFF',
    borderColor: '#21487D',
    desabled: '#282D49',
    deisabledBorderColor: '#2E3F55',
    deisabledColor: '#58677C',
    pleceHolderColor: '#89A7CE',
  },
  typographies: {
    error: '#FF0202',
    subtitle: '#89A7CE',
    success: '#008000',
  },
}

export type ThemeType = typeof lighTheme